...
코어의 개수를 늘리고, 대기하고 있는 Application Processor(AP) 들을 활성화

startap 커맨드를 통해 활성화된 코어들이 각자 자신의 APIC ID와 함께 메세지를 출력하도록 함

인터럽트를 여러 코어로 분산해서 처리하는데 기반이 되는 대칭 I/O 모두 구현

startsymmetricio 커맨드를 추가하여 기본의 인터럽트를 처리했던 PIC컨트롤러에서 대칭 I/O 모드로 전환

showirqintinmap 커맨드를 통해 I/O APIC를 관리하는 자료구조에 저장된 IRQ->INTIN 매핑 테이블 출력
